...
/**
 * @author Weiguang Chen
 * @E-mail weiguang978@163.com
 * @time  2015-12-1 下午3:31:45
 * @version V1.0
 * @remark 
 */
...
public class FileUtils {
	 ...
	 * 重命名文件或文件夹
	 * @param file 需要重命名的文件
	 * @param newName 新的名字，只有名字，不带目录路径
	 */
	    public static void rename(File file, String newName) {
			...
			if (file  == null || !file.exists()) {
				...
				System.out.println(newfile.getAbsoluteFile() + "已经存在！");
			}else {
...
//	    	String strPath = "C:\\Users\\ad\\Desktop\\com\\1.txt";
//	    	try {
//				readFile1(strPath);
//			} catch (Exception e) {
	           ...
	           // System.out.println("以行为单位读取文件内容，一次读一整行：");
	            ...
	            while ((lineString = reader.readLine()) != null) {
	            	...
	            	if(temp!=null && temp.length >0){
	            		listKey.add(temp[0].trim());
	            	}
	            	String[] temp1;
	            	if(temp.length >1){
	            		temp1 = temp[1].split("#");
	            		listValue.add(temp1[0].trim());
	            		
            			for(int i = 1; i<temp1.length-1;i++){
            				listValue.add(temp1[i].trim());
            			}	            		
	            		if(temp1.length >1){
            			listComment.add(temp1[temp1.length-1]);
	            		}else{
	            			listComment.add("");
	            		}
	            	}else{
